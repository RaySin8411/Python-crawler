...
def create_csv():
    with open('top_1000.csv', 'w', encoding="utf-8") as csvfile:
        ...
        for page in pages:
            ...
            for container in movie_containers:
                ...
                writer.writerow(
                    {'title': title, 'year': year, 'director': director, 'cast': cast, 'imdb_rating': rating})
...
def create_db():
    ...
    c = conn.cursor()
    c.execute('DROP TABLE IF EXISTS top_1000 ;')
    c.execute('CREATE TABLE top_1000 (title, year, director, cast, imdb_rating);')
    with open('top_1000.csv', 'r', encoding="utf-8") as csv_file:
